...
unsigned     copy;
{
	...
	if (state->window == Z_NULL) {
		state->window = (unsigned char FAR*)ZALLOC(strm,
	...
	for (;;)
		...
		case HCRC:
			if (state->flags & 0x0200) {
				NEEDBITS(16);
				if ((state->wrap & 4) &&
						hold != (state->check & 0xffff)) {
					strm->msg   = (char*)"header crc "
							     "mismatch";
					state->mode = BAD;
					break;
				}
		...
		case CODELENS:
			...
			ret = inflate_table(LENS, state->lens, state->nlen,
					...
					state->work);
			if (ret) {
				strm->msg   = (char*)"invalid literal/lengths "
						     "set";
				state->mode = BAD;
				break;
			}
		...
		case LEN:
			...
			if (here.op & 64) {
				strm->msg   = (char*)"invalid literal/length "
						     "code";
				state->mode = BAD;
				break;
			}
		...
		case CHECK:
			if (state->wrap) {
				...
				state->total += out;
				if ((state->wrap & 4) && out)
					strm->adler = state->check = UPDATE_CHECK(
							state->check,
							put - out, out);
				out = left;
				if ((state->wrap & 4) &&
		...
		case LENGTH:
			if (state->wrap && state->flags) {
				...
				if ((state->wrap & 4) &&
					...
					strm->msg = (char*)"incorrect length "
							   "check";
					state->mode = BAD;
					break;
				}
...
z_streamp source;
{
	...
	if (state->lencode >= state->codes &&
		...
		copy->distcode =
				copy->codes + (state->distcode - state->codes);
	}
