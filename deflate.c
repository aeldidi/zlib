...
const char deflate_copyright[] = " deflate 1.2.13 Copyright 1995-2022 "
...
#ifdef ZLIB_DEBUG
local void check_match OF(
		(deflate_state * s, IPos start, IPos match, int length));
#endif
...
#define INSERT_STRING(s, str, match_head)                                     \
	(UPDATE_HASH(s, s->ins_h, s->window[(str) + (MIN_MATCH - 1)]),        \
			match_head = s->prev[(str)&s->w_mask] =               \
					s->head[s->ins_h],                    \
			s->head[s->ins_h] = (Pos)(str))
#endif
...
#define CLEAR_HASH(s)                                                         \
	do {                                                                  \
		...
		zmemzero((Bytef*)s->head, (unsigned)(s->hash_size - 1) *      \
							  sizeof(*s->head));  \
	} while (0)
...
/* ========================================================================= */
int ZEXPORT
deflateInit2_(strm, level, method, windowBits, memLevel, strategy, version,
		stream_size)
z_streamp   strm;
int         level;
int         method;
...
int         stream_size;
{
	...
	if (memLevel < 1 || memLevel > MAX_MEM_LEVEL || method != Z_DEFLATED ||
			...
			level > 9 || strategy < 0 || strategy > Z_FIXED ||
			(windowBits == 8 && wrap != 1)) {
		return Z_STREAM_ERROR;
	}
	if (windowBits == 8)
...
int       strategy;
{
	...
	if ((strategy != s->strategy ||
		...
		if (err == Z_STREAM_ERROR)
			return err;
		if (strm->avail_in ||
				(s->strstart - s->block_start) + s->lookahead)
			return Z_BUF_ERROR;
	}
	if (s->level != level) {
...
uLong     sourceLen;
{
	...
	if (s->w_bits != 15 || s->hash_bits != 8 + 7)
		return (s->w_bits <= s->hash_bits ? fixedlen : storelen) +
		       wraplen;
...
int       flush;
{
	...
	if (s->status == GZIP_STATE) {
		...
		if (s->gzhead == Z_NULL) {
			...
			put_byte(s, (s->gzhead->text ? 1 : 0) +
							(s->gzhead->hcrc ? 2
									 : 0) +
							(s->gzhead->extra == Z_NULL ? 0
										    : 4) +
							(s->gzhead->name == Z_NULL ? 0
										   : 8) +
							(s->gzhead->comment == Z_NULL ? 0
										      : 16));
			...
			if (s->gzhead->extra != Z_NULL) {
				...
				put_byte(s, (s->gzhead->extra_len >> 8) &
								0xff);
			}
			if (s->gzhead->hcrc)
				strm->adler = crc32(strm->adler,
	...
	if (s->status == EXTRA_STATE) {
		if (s->gzhead->extra != Z_NULL) {
			...
			uInt left = (s->gzhead->extra_len & 0xffff) -
				    s->gzindex;
...
z_streamp source;
{
	...
	ds->pending_out =
			ds->pending_buf + (ss->pending_out - ss->pending_buf);
...
local void fill_window(s) deflate_state* s;
{
	...
	do {
		...
		if (s->strstart >= wsize + MAX_DIST(s)) {
			...
			zmemcpy(s->window, s->window + wsize,
					(unsigned)wsize - more);
			s->match_start -= wsize;
...
int            flush;
{
	...
	do {
		...
		if (len > (ulg)left + s->strm->avail_in)
			len = left +
			      s->strm->avail_in; /* limit len to the input */
		if (len > have)
		...
		if (left) {
			if (left > len)
				left = len;
			zmemcpy(s->strm->next_out, s->window + s->block_start,
					left);
	...
	if (used) {
		...
		if (used >= s->w_size) { /* supplant the previous history */
			...
			zmemcpy(s->window, s->strm->next_in - s->w_size,
					s->w_size);
			s->strstart = s->w_size;
			s->insert   = s->strstart;
		} else {
			if (s->window_size - s->strstart <= used) {
				...
				s->strstart -= s->w_size;
				zmemcpy(s->window, s->window + s->w_size,
						s->strstart);
				if (s->matches < 2)
	...
	if (left >= min_block ||
		...
		last = flush == Z_FINISH && s->strm->avail_in == 0 &&
				       ...
				       ? 1
				       : 0;
		_tr_stored_block(s, (charf*)s->window + s->block_start, len,
				last);
		s->block_start += len;
		flush_pending(s->strm);
	}
